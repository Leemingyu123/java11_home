...
public class StringBufferExam2 {
	...
	public static void main(String[] args) {
		...
		for(byte c : b) {	//문자 배열을 문자열로 변경
			char k = (char)c;
			sb.append(k);	//한 문자씩 StringBuffer에 추가
		}
		...
		System.out.println(cs.list()+", 글자수 : "+cs.list().length);		//문자배열로 length
		System.out.println(cs.toString()+", 글자수 : "+cs.toString().length());	//문자열로 length()
