...
class Outer{	//외부 클래스
	...
	class Inner {	//내부 클래스
		...
		void inMethod() {
			System.out.println("내부 클래스에서 외부 클래스의 맴버변수 접근: num"+num);
			System.out.println("내부 클래스에서 외부 클래스의 정적맴버변수 접근: sNum"+sNum);
		}
		
	}
	public void outMethod() {	//외부 클래스에서 내부 클래스의 맴버 메소드 호출
		inner.inMethod();
	}
}


public class InnerClassExam {	//타클래스
